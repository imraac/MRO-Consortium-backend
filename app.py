...
class Login(Resource):
    ...
    "user": {
        **user.to_dict(),
        "is_profile_complete": user.is_profile_complete or False  # Ensure this is always boolean
    },
    "access_token": access_token,
    "redirect": "/complete-profile" if not user.is_profile_complete else "/dashboard",
    "success": True,
    "message": "Login successful, please complete your profile" if not user.is_profile_complete else "Login successful"
}, 200)
...
def get_agency_info():
    ...
    if not agency:
        return jsonify({"message": "No agency information found"}), 404

    return jsonify(agency.as_dict()), 200
